...
async def create_on_join(member, before, after, bot):
    ...
    bot.logger.debug(f"{member} joined creator channel {after.channel} in guild '{guild_name}'")
    ...
    if db_creator_channel_info.child_overwrites == 1:
        ...
        bot.logger.debug(f"Using creator channel overwrites for temp channel in {creator_channel.id} in guild '{guild_name}'")
        ...
        overwrites = {}
        bot.logger.debug(f"Using no inherited overwrites for temp channel in {creator_channel.id} in guild '{guild_name}'")
    ...
    try:
        ...
        bot.logger.debug(f"Moved {member} to {new_temp_channel} in guild '{guild_name}'")
    ...
    bot.logger.debug(f"Registered temp channel {new_temp_channel.id} in database for owner {member.id} in guild '{guild_name}'")
    channel_name = create_temp_channel_name(bot, new_temp_channel, db_creator_channel_info=db_creator_channel_info)
    bot.logger.debug(f"Generated temp channel name '{channel_name}' for {new_temp_channel.id} in guild '{guild_name}'")
    ...
    bot.logger.debug(f"Sent create logs for temp channel {new_temp_channel.name} ({new_temp_channel.id}) in guild '{guild_name}'")


async def delete_on_leave(member, before, after, bot):
    old_temp_channel = before.channel
    guild_name = member.guild.name
    bot.logger.debug(f"{member} left temp channel {old_temp_channel.name} ({old_temp_channel.id}) in guild '{guild_name}'")

    if len(old_temp_channel.members) >= 1:
        bot.logger.debug(f"Temp channel {old_temp_channel.id} still has {len(old_temp_channel.members)} member(s) in guild '{guild_name}', skipping delete")
        return

    bot.logger.debug(f"Left temp channel is empty in guild '{guild_name}'. Deleting...")

    try:
        await old_temp_channel.delete()
        bot.repos.temp_channels.remove(old_temp_channel.id)
        bot.logger.debug(f"Deleted {old_temp_channel.name} in guild '{guild_name}'")

    except discord.NotFound as e:
        bot.repos.temp_channels.remove(old_temp_channel.id)
        bot.logger.debug(f"Channel not found removing entry in db in guild '{guild_name}', handled. {e}")
        return
        ...
        bot.logger.error(f"Unknown error removing temp channel in guild '{guild_name}'. {e}")
        return
    ...
    bot.logger.debug(f"Sent remove logs for temp channel {old_temp_channel.name} ({old_temp_channel.id}) in guild '{guild_name}'")
